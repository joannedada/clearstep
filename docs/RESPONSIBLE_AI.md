...
- All model outputs are schema-validated on the server-side before reaching the user; missing fields, inacurate variable types, and empty task lists are caught and rejected
...
- **Upload content screening:** All uploaded file content is screened through Azure Content Safety (all 4 categories at lower thresholds), Prompt Shields, and a cyber abuse regex before any text is returned to the frontend or passed to the AI pipeline. Files are never stored, only extracted in memory and discarded
...
- Conditional medical instructions ("if you miss a dose", "if it is almost time", "unless", "in case") are classified as warnings, not tasks and are enforced in both the prompt and the Python validator
...
- Prompt injection evaluated against 14 prompt injection vectors; All results flagged as 'High Risk' or 'Caution,' failing to meet compliance standards
- Fallback mode ensures the application returns no output. A mandatory indicator bar confirms fallback execution, ensuring keyword-matching is never silently presented as AI-driven results
...
- Five colour palettes built for specific neurological needs:
  ...
  - High focus (ADHD): single accent colour from teal-blue family, no competing visual elements
  - Dark mode: muted variants across all states, no blinding contrast shifts
  - Calm default: off-white, non-aggressive teal accent
- Three reading levels (Big / Normal / Small) control font size, line height, **and** AI output density providing a tailored user experience
- Medical content never receives a "CLEAR" badge regardless of model output — enforced in `renderResult()` in `index.html`
- **Multilingual equity:** Azure AI Language detects input language on every request. If non-english is detected, Claude responds entirely in that language across all fields. No configuration or extra steps required from the user
- **No enforced timers.** For users in cognitive overload, urgency adds anxiety. Optional reminders replace enforced pacing
- **File attachment:** Users can attach .txt, .pdf, .docx, or image files (.png, .jpg, .jpeg) to bypass the copy-paste requirement. Reduces the barrier for users with motor difficulties or varying levels of  digital literacy
- **Mobile-first responsive design:** Full experience works on mobile browsers supporting both modes, all palettes, task engine, reminders and upload capability

---

## Transparency

- "Why this result?" explainability panel on every output in plain language, no technical jargon
...
- Reading level labels (Big / Normal / Small) in plain language, not technical jargon
...
- No user message content is stored in the system . Blob Storage logs contain AI response JSON only, never input text
...
- Severity Level ≥ 4+ triggers a default response for  human support services like the 988 Lifeline, ensuring the AI model is never used for acute crisis intervention
