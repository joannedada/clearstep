...
- File upload blocks are logged separately with category and severity — proving the upload safety layer is active independently of the analyze pipeline
...
- Conditional medical instructions ("if you miss a dose", "if it is almost time", "unless", "in case") classified as warnings, never tasks — enforced in both the prompt and the Python validator
- Leaked safety rules in task lists (e.g. "Do not crush tablet", "Skip dose") detected by pattern matching and moved to the warnings array — users never see safety rules as action steps
...
- Fallback mode ensures the app never returns nothing. A visible indicator bar always shown when fallback runs — app never silently presents keyword-matching as AI analysis
...
- **Fallback transparency:** When AI is unavailable and keyword scoring runs instead, a visible caution bar appears: "AI unavailable — showing basic analysis only." User always knows which type of result they received
...
- Medical and legal content always defers to the original document and a real professional — enforced in prompt and persistent disclaimer bar
- Crisis response directs users to human services (988 Lifeline) — model is never involved at severity ≥ 4
- "Always consult a professional" present in every medical result
- Step-by-step task engine never auto-advances — user controls every transition, no decision made for them
...
- Prompt Shields detects jailbreaks at Azure infrastructure level before any LLM is called
...
- CORS policy: Flask-CORS restricts API to ClearStep domain only
...
| Support efficient correction | "Check another" resets cleanly. Undo available at every step. Batch tasks allow continuation without restart. |
